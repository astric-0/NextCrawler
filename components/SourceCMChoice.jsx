...
const SourceCMChoice = ({ label, xs }) => {
	...
	const moduleBtnList = getContentModulePacks().map((modulePack, index) => {
	...
	return (
		<Form.Group>
			<Form.Label as={Col} xs={xs} className="mt-3">
				{label}
			</Form.Label>
			<ButtonGroup className="w-100">{moduleBtnList}</ButtonGroup>
		</Form.Group>
	);
};
