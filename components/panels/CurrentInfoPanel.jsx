...
const keyNames = [
	...
	{
		key: "remaining",
		name: "Remaining URLs in Current Depth",
		variant: "primary",
	},
	{
		key: "collected",
		name: "URLs Collected for Next Depth",
		variant: "dark",
	},
...
const CurrentInfoPanel = (_) => {
	...
	const infoList = keyNames.map(({ key, name, variant }) => (
		<Col key={key} className="my-2" md="12" lg="3">
			<Container className="rounded">
				<Button className="shadow w-100" variant={variant} size="lg">
					{info[key] ?? 0}
				</Button>
	...
	return (
		<Container className="my-3 text-primary">
			...
			<Row className="mt-3">{infoList}</Row>
		</Container>
	);
};
